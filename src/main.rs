...
struct LoopContext {
    action: Cell<Option<LoopAction>>,
    app_context: Rc<RefCell<Context>>,
}
...
fn main() -> std::io::Result<()> {
    ...
    let mut event_loop = calloop::EventLoop::<LoopContext>::try_new().unwrap();

    setup_keyboard_handler(&env, &event_loop);

    WaylandSource::new(queue)
        .quick_insert(event_loop.handle())
        .unwrap();

    // our loop context to maintain actions
    let mut context = LoopContext::new(context);

    loop {
        // This is ugly, let's hope that some version of drain_filter() gets stabilized soon
        // https://github.com/rust-lang/rust/issues/43244
        {
            let redraw = context.action.take() == Some(LoopAction::Redraw);
        ...
        event_loop.dispatch(None, &mut context).unwrap();
    }
}

fn setup_keyboard_handler(env: &Environment<RMenuEnv>, event_loop: &EventLoop<LoopContext>) {
    let mut seats = Vec::<(
        String,
            ...
            if has_kbd {
                ...
                match map_keyboard_repeat(
                    ...
                    move |event, _, mut dispatch_data| {
                        ...
                        if handle_keyboard_event(event, &seat_name, context) {
                            // apply filter
...
fn handle_keyboard_event(event: KbEvent, seat_name: &str, context: &Rc<RefCell<Context>>) -> bool {
    match event {
        ...
        KbEvent::Key {
            ...
            if state == KeyState::Pressed {
                handle_key(&context, rawkey, utf8)
            } else {
                false
            }
        }
...
fn handle_key(context: &Rc<RefCell<Context>>, rawkey: u32, utf8: Option<String>) -> bool {
    match rawkey {
        /* ESC */
        1 => {
            // exit on ESC pressed
